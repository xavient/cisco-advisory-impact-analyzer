...
- **`uv` is the recommended prerequisite**: The primary experience assumes `uv` is installed and on PATH; installing `uv` itself is out of scope and covered only by documentation. For environments where `uv` cannot be installed, a Python-only fallback is supported — installing from a source checkout with `pip install .` into a virtual environment (see the "no-uv" path in the README).
- **Distribution replaces the old flow**: uv is the primary supported distribution and run mechanism; the previous `install.py` / `run.py` / in-repo self-updater and clone-based launcher scripts are retired, with the README, `docs/index.html`, and constitution updated to match (see FR-024). A `pip install .` from a source checkout remains a supported fallback (no bespoke installer scripts return); `--update` is uv-specific and does not apply on that path. This is a breaking change for anyone on the old clone-based flow, who must reinstall via uv or the pip fallback.
