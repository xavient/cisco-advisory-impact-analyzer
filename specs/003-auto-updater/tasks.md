...
**Purpose**: Shared primitives used by **every** user story — read the installed version,
compare versions, and resolve the latest release. **No user story work can begin until this
phase is complete.**

- [ ] T005 In `updater.py`, implement `read_installed_version()` reading the root `VERSION`; a missing/unparseable file yields *unknown* (sorts as older; never crashes) (FR-003, data-model).
- [ ] T006 In `updater.py`, implement `parse_version()` and `compare_versions()` — numeric, component-wise; strip a leading `v`; missing components → `0` (`1.2` == `1.2.0`); unknown sorts lowest; must order `1.10.0` > `1.9.0` (FR-004, spec Clarifications Q3).
- [ ] T007 In `updater.py`, implement `resolve_latest()` — GitHub Releases API `tag_name` (send `Accept: application/vnd.github+json` + `User-Agent`; `urllib` honors `HTTP(S)_PROXY`), with a fallback that reads the tag from the `…/releases/latest` redirect URL on HTTP 403/rate-limit/unreachable; return `(tag, reachable)` (FR-004, FR-013 proxy, FR-015, research D2).
- [ ] T008 In `updater.py`, implement `check_update()` combining installed + latest into an Update Check Result: `up_to_date` | `update_available` | `ahead` | `latest_unknown` (FR-004, FR-005, data-model).
- [ ] T009 Create `tests/test_updater.py` with offline unit tests for `parse_version`/`compare_versions` (equal, older, ahead, `v`-prefix, `1.10.0` > `1.9.0`, unknown) and `read_installed_version` (present / missing / garbage) (Constitution testability; quickstart "Automated checks").

**Checkpoint**: `python3 -m unittest tests.test_updater` passes; version compare + `check_update()` behave correctly offline (with a stubbed latest).
...
- [ ] T010 [US1] In `updater.py`, implement `download_release(tag)` — download the **tag-pinned** asset `cisco-advisory-impact-analyzer.zip` and its sibling `…zip.sha256` over HTTPS into a temp dir (`urllib`, proxy-honored); actionable error on failure (FR-014, research D3).
- [ ] T011 [US1] In `updater.py`, implement `verify_package()` — SHA-256 of the zip equals the published digest, the archive is a well-formed zip (`ZipFile` + `testzip()`), and its embedded root `VERSION` equals the tag; extract to staging and locate the package root (dir containing `VERSION`, depth ≤ 2). Any failure ⇒ no files changed (FR-006, research D3/D11).
- [ ] T012 [US1] In `updater.py`, implement `create_backup()` — before any change, copy every file the update will overwrite or remove (plus the current `VERSION`/`MANIFEST`) into `.update-backup/<old>-<timestamp>/`, **never** touching the preserve-list (FR-011, research D4/D8).
- [ ] T013 [US1] In `updater.py`, implement `apply_update()` — write the `.update-in-progress` marker; overlay staged files skipping `PRESERVE` / `.git/` / `.update-backup/` / the marker; apply manifest-diff deletions (old ∖ new, minus preserve-list); write the new `VERSION`/`MANIFEST`; clear the marker (FR-002, FR-012 marker, FR-019, research D4/D5).
- [ ] T014 [US1] In `updater.py`, implement `refresh_deps_if_changed()` — compare old vs new `requirements.txt`; if changed and a `.venv` exists, run the venv interpreter (`.venv/Scripts/python.exe` on Windows, `.venv/bin/python` otherwise) `-m pip install -r requirements.txt`; if no `.venv`, print an actionable hint to run `install.py` (FR-007, research D6).
- [ ] T015 [US1] Extend `tests/test_updater.py` with offline tests for the manifest add/remove diff (old ∖ new) and preserve-list exclusion (pure functions; no network/filesystem side effects) (Constitution testability).
- [ ] T016 [US1] Create `update.py` — argparse CLI (default update path, `--yes`/`-y`, `--repo`, `--help`) that orchestrates check → show `current → new` → `ui.confirm` (skipped by `--yes`) → download → verify → backup → apply → refresh deps → report new version; up-to-date/ahead short-circuit with no changes; actionable `ui.fail` errors and exit codes `0/1/2/3/4` with `130` on Ctrl-C via a `run_cli`-style handler (FR-001, FR-005, FR-009, FR-018, contracts/cli.md, research D14). Rollback/auto-revert/recovery are added in US3.
...
- [ ] T017 [P] [US2] In `update.py`, add `--check` mode — print `Installed` / `Latest` / `Status` from `check_update()` (`up to date` | `update available` | `ahead of latest` | `latest unknown`) and make **no** changes; exit codes per contracts/cli.md (FR-008).
- [ ] T018 [P] [US2] In `run.py`, add `--version` handling **before** delegating to the analyzer — print `read_installed_version()` and exit `0`; the flag is consumed by `run.py`, not passed through (FR-010, contracts/cli.md).
...
- [ ] T019 [US3] In `updater.py`, implement `restore_backup()` — restore the newest `.update-backup/…` over the install (skipping the preserve-list) and restore the prior `VERSION`/`MANIFEST` (FR-011).
- [ ] T020 [US3] In `updater.py`, wire **auto-revert**: if `apply_update()` (or the finalize step) raises, call `restore_backup()`; return distinct outcomes for revert-succeeded vs revert-failed so the CLI can exit `5` vs `6` (FR-012, research D14).
- [ ] T021 [US3] In `updater.py`, implement **startup recovery**: if `.update-in-progress` exists, restore from the recorded backup before doing anything else (FR-012, data-model recovery).
- [ ] T022 [US3] In `updater.py`, implement backup **retention pruning** — on a successful update keep only the last 2 backups (research D8).
- [ ] T023 [US3] In `update.py`, add the `--rollback` command (restore newest backup, report reverted version, exit `1` if none), call startup recovery at entry, and surface the auto-revert exit codes `5`/`6` from the update path (FR-011, FR-012, contracts/cli.md, research D14).
...
**Purpose**: Release automation (required — FR-020/FR-021), documentation parity, governance,
and full validation.

- [ ] T026 [P] Create `.github/workflows/release.yml` — on a version-tag push: stamp `VERSION` = tag, regenerate `MANIFEST`, build the flat `cisco-advisory-impact-analyzer.zip` (runtime files only, per research D12 excludes), compute `cisco-advisory-impact-analyzer.zip.sha256`, and upload both as release assets (FR-020, FR-021, contracts/release-artifacts.md).
- [ ] T027 [P] Update `README.md` to document `python3 update.py`, `--check`, `--yes`, `--rollback`, `python3 run.py --version`, and the `--no-update-check` / `CAIA_NO_UPDATE_CHECK` opt-out (Constitution Documentation gate).
- [ ] T028 [P] Update `docs/index.html` with an in-place update note (`python3 update.py`), mirroring the README so the two stay consistent (Constitution Documentation gate).
- [ ] T029 [P] Amend `.specify/memory/constitution.md` to add GitHub (`api.github.com`, `github.com`, `objects.githubusercontent.com`) to the allowed external endpoints for the updater, with a Sync Impact Report and a version bump — recording the reviewed change (plan Constitution Check, research D13).
...
- **User Stories (Phases 3–6)**: All depend on Foundational. US1 → US2 → US3 mostly extend
  ...
  `updater.py`.
- **Polish (Phase 7)**: Depends on the desired user stories being complete (T026 release
...
- Tests (where included) sit next to the logic they cover: semver/VERSION tests in
  Foundational (T009); manifest-diff/preserve tests in US1 (T015).
- `updater.py` library functions before the `update.py`/`run.py` code that calls them.
- US1 order: download (T010) → verify (T011) → backup (T012) → apply (T013) → deps (T014) →
  CLI (T016).

### Parallel Opportunities

- **Setup**: T002 (`updater.py`), T003 (`.gitignore`), T004 (`MANIFEST`) are different files → `[P]`.
- **US2**: T017 (`update.py`) and T018 (`run.py`) are different files with no interdependency → `[P]`.
- **Polish**: T026 (`release.yml`), T027 (`README.md`), T028 (`docs/index.html`), T029
  (`constitution.md`) are different files → `[P]`. T030 (validation) runs last.
- Foundational and each story's `updater.py` edits are sequential (same file).
...
Task: "T026 Create .github/workflows/release.yml"
Task: "T027 Update README.md for the update commands"
Task: "T028 Update docs/index.html with the update note"
Task: "T029 Amend constitution.md (GitHub endpoint)"
```
...
2. Phase 2: Foundational (T005–T009) — **blocks everything**
3. Phase 3: User Story 1 (T010–T016)
...
6. Polish → release automation (T026), docs parity (T027/T028), governance (T029), full
   quickstart + release validation (T030)

---

## Notes

- Targeted tests only (pure logic): `tests/test_updater.py` (T009, T015). Network/apply/
